...
fn custom_args_with_one_remaining() {
    let (config, mut remaining) = config::Config::custom_args_and_optional_files(&["custom_args", "--foo", "42", "remaining_arg"], iter::empty::<PathBuf>()).unwrap();
    assert_eq!(config.foo, Some(42));
...
fn custom_args_with_two_remaining() {
    let (config, mut remaining) = config::Config::custom_args_and_optional_files(&["custom_args", "--foo", "42", "remaining_arg", "another_arg"], iter::empty::<PathBuf>()).unwrap();
    assert_eq!(config.foo, Some(42));
    assert_eq!(remaining.next(), Some("remaining_arg".into()));
